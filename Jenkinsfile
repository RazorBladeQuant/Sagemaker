pipeline {
    ...
    stages {
        stage('Docker Build') {
            ...
            steps {
                    sh "curl -s -L https://nvidia.github.io/nvidia-docker/gpgkey | \"
                    sh "sudo apt-key add -"
                    sh "distribution=$(. /etc/os-release;echo $ID$VERSION_ID)"
                    sh "curl -s -L https://nvidia.github.io/nvidia-docker/$distribution/nvidia-docker.list | \"
                    sh "sudo tee /etc/apt/sources.list.d/nvidia-docker.list"
